...
service-up:
	@set -eu; \
	keysets_json="$$(BTC_ARTIFACT_FILE="$(BTC_ARTIFACT)" ETH_ARTIFACT_FILE="$(ETH_ARTIFACT)" SERVICE_KS_BTC_REGTEST="$(SERVICE_KS_BTC_REGTEST)" SERVICE_KS_BTC_TESTNET="$(SERVICE_KS_BTC_TESTNET)" SERVICE_KS_ETH_SEPOLIA="$(SERVICE_KS_ETH_SEPOLIA)" SERVICE_KS_ETH_LOCAL="$(SERVICE_KS_ETH_LOCAL)" SERVICE_KS_BTC_REGTEST_INDEX0_ADDRESS="$(SERVICE_KS_BTC_REGTEST_INDEX0_ADDRESS)" SERVICE_KS_BTC_TESTNET_INDEX0_ADDRESS="$(SERVICE_KS_BTC_TESTNET_INDEX0_ADDRESS)" SERVICE_KS_ETH_SEPOLIA_INDEX0_ADDRESS="$(SERVICE_KS_ETH_SEPOLIA_INDEX0_ADDRESS)" SERVICE_KS_ETH_LOCAL_INDEX0_ADDRESS="$(SERVICE_KS_ETH_LOCAL_INDEX0_ADDRESS)" $(SCRIPT_DIR)/service_keysets_json.sh)"; \
	PAYMENT_REQUEST_DEVTEST_KEYSETS_JSON="$$keysets_json" $(SCRIPT_DIR)/service_verify_keysets.sh; \
	PAYMENT_REQUEST_DEVTEST_KEYSETS_JSON="$$keysets_json" PAYMENT_REQUEST_KEYSET_HASH_HMAC_SECRET="$(SERVICE_KEYSET_HASH_HMAC_SECRET)" PAYMENT_REQUEST_KEYSET_HASH_HMAC_PREVIOUS_SECRETS_JSON='$(SERVICE_KEYSET_HASH_HMAC_PREVIOUS_SECRETS_JSON)' $(SERVICE_DC) up -d --build postgres app; \
	PAYMENT_REQUEST_DEVTEST_KEYSETS_JSON="$$keysets_json" PAYMENT_REQUEST_KEYSET_HASH_HMAC_SECRET="$(SERVICE_KEYSET_HASH_HMAC_SECRET)" PAYMENT_REQUEST_KEYSET_HASH_HMAC_PREVIOUS_SECRETS_JSON='$(SERVICE_KEYSET_HASH_HMAC_PREVIOUS_SECRETS_JSON)' SERVICE_COMPOSE_FILE="$(SERVICE_COMPOSE)" LOCAL_SERVICE_PROJECT="$(SERVICE_PROJECT)" ETH_ARTIFACT_FILE="$(ETH_ARTIFACT)" $(SCRIPT_DIR)/service_sync_catalog.sh
